...
con=pymysql.connect(host=host,user=username,passwd=password,database=database)
...
DBS_clf = DBSCAN(eps=1, min_samples=10)
...
X_train=[[float(i)] for i in  video_sentiments_df['_like'][:-3000]]
X_test=[[float(i)] for i in  video_sentiments_df['_like'][-3000:]]
Y_train=[[j] for j in [(i-5)*(i-5) for i in video_sentiments_df['sentiment'][:-3000]]]
Y_test=[[j] for j in [(i-5)*(i-5) for i in video_sentiments_df['sentiment'][-3000:]]]

reg_1=pl.fit_transform(X_train)
reg_2=pl.fit_transform(X_test)
# print([[i] for i in  video_sentiments_df['_like']])-
reg=lm.fit(X=reg_1,y=[[j] for j in Y_train])
```
##### 8.测试集预测结果
```python
mapr=map(lambda a,b:a/b,reg.predict(X_test),Y_test)
print(reduce(lambda a,b:a+b,mapr)/len(mapr))
```
|结果|0.702351|
|:--:|:--:|
##### 9.可视化回归结果
...
# 结论
可知各话题下情感倾向趋于平和，且视频综合热度与视频相关文本的总情感倾向有一定的关系，当情感倾向越强烈，播放量越低。情感倾向趋于适中时，综合热度高。
